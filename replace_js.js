...
const path = 'C:\\Users\\Acer\\deploy-ready\\public\\index.html';
let content = fs.readFileSync(path, 'utf-8');

// Find the start and end markers
const start_marker = '/* ============ API HELPERS ============ */';
const end_marker = `  saving=false;
...
const old_block = content.slice(start_idx, end_idx);

const new_block = `/* ============ LOCALSTORAGE MIGRATION (one-time) ============ */
...
content = content.slice(0, start_idx) + new_block + content.slice(end_idx);
fs.writeFileSync(path, content, 'utf-8');
console.log('Done. Old block length:', old_block.length, 'New block length:', new_block.length);